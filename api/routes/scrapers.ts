...
router.post('/:id/start', authenticate, async (req: AuthRequest, res: express.Response) => {
  try {
    ...
    scraperService.scrapeReviews(parseInt(id)).then(result => {
      console.log(`Scraping completed for scraper ${id}:`, result);
    }).catch(error => {
      console.error(`Scraping failed for scraper ${id}:`, error);
    });
